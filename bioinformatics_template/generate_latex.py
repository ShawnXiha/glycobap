#!/usr/bin/env python3
"""Generate complete LaTeX paper from markdown source."""
...
AlphaFold 3 (AF3) represents a breakthrough in biomolecular structure prediction \citep{abramson2024}. However, \citet{huang2025} demonstrated that AF3's standard input formats fail to preserve glycan stereochemistry. SMILES format achieves only ${\sim}62\%$ stereochemistry accuracy, while the correct CCD+BAP format requires manual atom-by-atom specification taking 30--60 minutes per structure.

We present GlycoSMILES2BAP, an automated pipeline that bridges the gap between standard glycan notations and AF3's stereochemistry-preserving input format.
